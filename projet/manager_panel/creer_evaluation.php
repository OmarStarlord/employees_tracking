...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Generate a unique ID using a combination of user ID and date
    $unique_id = $employee_id . '_' . date('YmdHis');

    // Define the target directory
    $target_dir = "uploads/";

    // Create a directory with the unique ID if it doesn't exist
    if (!file_exists($target_dir . $unique_id)) {
        mkdir($target_dir . $unique_id, 0777, true); // Create directory recursively
    }

    // Append the unique ID to the target directory
    $target_dir .= $unique_id . '/';

    // Append the file name to the target directory
    $target_file = $target_dir . basename($_FILES["evaluation_form"]["name"]);
